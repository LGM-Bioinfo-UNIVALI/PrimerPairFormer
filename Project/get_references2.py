...
if __name__ == '__main__':
	primers = pd.read_csv('primers_reference_test.csv', sep=';')
	...
	for primer_id, primer_code, primer_seq in zip(primers['id'], primers['code'], primers['nuc']):
		if primer_id in filtered_primers:
			...
			# 	else:
			# 		if not os.path.exists(f"get_references2/{primer_code}"):
			# 			os.makedirs(f"get_references2/{primer_code}")
			...
			# 		create_primer_fasta(primer_seq_extended, f'get_references2/{primer_code}/{primer_code}.fa')

			# 		if len(primer_seq_extended) < 5:
			# 			max_target_seqs = str(int(ceil(50 / len(primer_seq_extended))))
			# 		else:
			# 			max_target_seqs = '10'

			# 		run_blast(f'./get_references2/{primer_code}/{primer_code}.fa', f'./get_references2/{primer_code}/{primer_code}_nt.tsv', max_target_seqs=max_target_seqs, num_threads='12')

					
			# 		run_blast(f'./get_references2/{primer_code}/{primer_code}.fa', f'./get_references2/{primer_code}/{primer_code}_16S.tsv', max_target_seqs=max_target_seqs, num_threads='12', database='/media/bioinfo/6tb_hdd/04_Blast_Databases/16sDB/16S_ribosomal_RNA')

			# 		df_primer_references_nt = pd.read_csv(f'get_references2/{primer_code}/{primer_code}_nt.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'])
			# 		df_primer_references_16S = pd.read_csv(f'get_references2/{primer_code}/{primer_code}_16S.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'])

			# 		df_primer_references = pd.concat([df_primer_references_nt, df_primer_references_16S]).reset_index(drop=True)

			# 		df_primer_references = df_primer_references.sort_values(by=['qcovs', 'pident', 'slen'], ascending=[False, False, False]).reset_index(drop=True)
			...
			# 			try:
			# 				subprocess.run(
			# 					f'cd ./get_references2/{primer_code}/ &&\
	...
	# if not os.path.exists(f"get_references2/individual_seqs"):
	# 	os.makedirs(f"get_references2/individual_seqs")
	# create_individual_seqs_fasta(individual_seqs, f'get_references2/individual_seqs/individual_seqs.fa')

	
	# run_blast(f'./get_references2/individual_seqs/individual_seqs.fa', f'./get_references2/individual_seqs/individual_seqs_nt.tsv', max_target_seqs='50', num_threads='12')

	# run_blast(f'./get_references2/individual_seqs/individual_seqs.fa', f'./get_references2/individual_seqs/individual_seqs_16S.tsv', max_target_seqs='50', num_threads='12', database='/media/bioinfo/6tb_hdd/04_Blast_Databases/16sDB/16S_ribosomal_RNA')


	# df_references_nt = pd.read_csv(f'get_references2/individual_seqs/individual_seqs_nt.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'])
	# df_references_16S = pd.read_csv(f'get_references2/individual_seqs/individual_seqs_16S.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'])

	# df_references = pd.concat([df_references_nt, df_references_16S]).reset_index(drop=True)
	...
	# 	if not os.path.exists(f"get_references2/individual_seqs/{primer_code}"):
	# 		os.makedirs(f"get_references2/individual_seqs/{primer_code}")
			
	# 		df_primer_references = df_references.loc[df_references['qseqid'] == primer_code].reset_index(drop=True)
	...
	# 			try:
	# 				subprocess.run(
	# 					f'cd ./get_references2/individual_seqs/{primer_code}/ &&\
	# 					blastdbcmd -db /media/bioinfo/6tb_hdd/04_Blast_Databases/BLAST_DB_nt/nt -entry {sgi} > {organism_renamed}.fasta',
	# 					shell=True,
	# 					executable='/bin/bash'
	# 				)
