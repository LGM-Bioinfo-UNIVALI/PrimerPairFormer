...
def run_blast(fasta_file, output_file, max_target_seqs='10', num_threads='6'):
	subprocess.run(
...
if __name__ == '__main__':
	...
	run_blast(f'./get_references/individual_seqs/individual_seqs.fa', f'./get_references/individual_seqs/individual_seqs.tsv', max_target_seqs='50', num_threads='12')

	df_primers_references = pd.read_csv(f'get_references/individual_seqs/individual_seqs.tsv', sep='\t', names=['qseqid', 'length', 'score', 'bitscore', 'pident', 'nident', 'evalue', 'gapopen', 'gaps', 'qcovs', 'qcovhsp', 'stitle', 'sscinames', 'mismatch', 'qstart', 'qend', 'sstart', 'send'])


	df_references = df_primers_references.sort_values(by='pident').reset_index(drop=True)
	primer_codes = df_references['qseqid'].unique()

	for primer_code in primer_codes:
		if not os.path.exists(f"get_references/individual_seqs/{primer_code}"):
			os.makedirs(f"get_references/individual_seqs/{primer_code}")
			
			df_primer_references = df_primers_references.loc[df_primers_references['qseqid'] == primer_code].reset_index(drop=True)
			...
			for organism in most_frequent['index']:
				...
				result_count_output = subprocess.run(f'esearch -db nuccore -query "({organism}[Organism] OR ({organism}[Organism] OR {organism}[All Fields])) AND complete[All Fields] AND genome[All Fields] AND mitochondrion[filter]" | xtract -pattern ENTREZ_DIRECT -element Count',capture_output = True, text = True, shell=True, executable='/bin/bash').stdout

				if result_count_output == '':
					print(f'esearch -db nuccore -query "({organism}[Organism] OR ({organism}[Organism] OR {organism}[All Fields])) AND complete[All Fields] AND genome[All Fields] AND mitochondrion[filter]" | xtract -pattern ENTREZ_DIRECT -element Count')
					result_count_output = 0
				else:
					result_count_output = int(result_count_output)


				if result_count_output > 2000:
					subprocess.run(
						f'cd ./get_references/individual_seqs/{primer_code}/ &&\
						esearch -db nucleotide -query "(({organism}[Organism] OR {organism}[All Fields]) AND complete[All Fields] AND genome[All Fields]) AND mitochondrion[filter]" | efetch -format docsum -stop 1000 | xtract -pattern DocumentSummary -element AccessionVersion Organism Title > search_results.tsv',
						shell=True,
						executable='/bin/bash'
					)
				elif result_count_output > 0:
					subprocess.run(
						f'cd ./get_references/individual_seqs/{primer_code}/ &&\
						esearch -db nucleotide -query "(({organism}[Organism] OR {organism}[All Fields]) AND complete[All Fields] AND genome[All Fields]) AND mitochondrion[filter]" | efetch -format docsum | xtract -pattern DocumentSummary -element AccessionVersion Organism Title > search_results.tsv',
						shell=True,
					...
					result_count_output = subprocess.run(f'esearch -db nuccore -query "({organism}[Organism] OR ({organism}[Organism] OR {organism}[All Fields])) AND (biomol_genomic[PROP] AND ("500"[SLEN] : "250000"[SLEN]))" | xtract -pattern ENTREZ_DIRECT -element Count',capture_output = True, text = True, shell=True, executable='/bin/bash').stdout

					if result_count_output == '':
						print(f'esearch -db nuccore -query "({organism}[Organism] OR ({organism}[Organism] OR {organism}[All Fields])) AND (biomol_genomic[PROP] AND ("500"[SLEN] : "250000"[SLEN]))" | xtract -pattern ENTREZ_DIRECT -element Count')
						result_count_output = 0
					else:
						result_count_output = int(result_count_output)
					...
					subprocess.run(
						f'cd ./get_references/individual_seqs/{primer_code}/ &&\
						esearch -db nucleotide -query "(({organism}[Organism] OR {organism}[All Fields]) AND (biomol_genomic[PROP] AND ("500"[SLEN] : "250000"[SLEN]))" | efetch -format docsum -stop 1000 | xtract -pattern DocumentSummary -element AccessionVersion,Gi,Slen,Organism,Title > search_results.tsv',
						shell=True,
						executable='/bin/bash'
					)
						...
						subprocess.run(
							f'cd ./get_references/individual_seqs/{primer_code}/ &&\
							esearch -db nucleotide -query "(({organism}[Organism] OR {organism}[All Fields]) AND (biomol_genomic[PROP] AND ("500"[SLEN] : "250000"[SLEN]))" | efetch -format docsum | xtract -pattern DocumentSummary -element AccessionVersion,Gi,Slen,Organism,Title > search_results.tsv',
							shell=True,
							executable='/bin/bash'
						)

					# uma outra maneira para obter a length da sequencia é essa, extraindo pelo campo Statistics
					# esearch -db nucleotide -query "((Heliamphora minor [Organism] OR Heliamphora minor [All Fields]) AND (biomol_genomic[PROP] AND ("500"[SLEN] : "50000"[SLEN]))" | efetch -format docsum | xtract -pattern DocumentSummary -element AccessionVersion,Organism,Title --STATS other_columns -block Stat -if "@type" -equals "Length" -position first -element @count > search_results.tsv

				search_results = pd.read_csv(f'get_references/individual_seqs/{primer_code}/search_results.tsv', sep='\t', names=['AccessionVersion', 'Organism', 'Title'])				
				selected_search_results = search_results.loc[search_results['Organism'] == organism]
				if len(selected_search_results) == 0:
					search_results = search_results.loc[search_results['Organism'].str.contains(organism)]
				else:
...
# se o search_results der zero, provavelmente eh alguma coisa que nao tem mitocondrial ou que tem poucas informações, ver como proceder

# os que deram menos do que 10 especies, talvez posso primeiro identificar a taxonomia completa e identificar aqueles que são bacteria, inseto, planta ou algo assim (verificar se inseto e planta também  melhor analisar como um banco separado)
# pode ser que tenha dado menos q 10 pq deu mts repetidos. posso primeiro eliminar tudo q deu de bacteria pelo nt, e depois juntar os resultados do nt com o de 16s

# os que sao degenerados, verificar primeiro quantas variantes existem, se existir menos que 5, dividir 50 pelo numero de variantes para determinar tamanho do blast, para termos pelo menos 50 resultados de blast

# verificar se os resultados do blast conferem com os que foram baixados

# oq tiver sp. cortar adiante