...
def download_references(df_references, primer_code, primer_seq, max_len, count, selected_organisms, path2save_references):
...
if __name__ == '__main__':
	path2save_references = '/media/bioinfo/6tb_hdd/03_ELLEN/02_data/primers_references2/'
	...
	# create_fasta(seqs, f'get_references3/primers_seqs.fa')
	...
	df_references_nt = pd.read_csv(f'get_references3/primers_seqs_nt.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'], dtype={'qseqid': 'str'})
	df_references_16S = pd.read_csv(f'get_references3/primers_seqs_16S.tsv', sep='\t', names=['qseqid', 'pident', 'gaps', 'qcovs', 'mismatch', 'evalue', 'score', 'bitscore', 'sgi', 'sacc', 'slen', 'stitle', 'sscinames'], dtype={'qseqid': 'str'})
	...
	for i in filtered_primers:
		if str(i) not in primer_codes:
			print(i)
	print(len(primer_codes))
	for primer_code in primer_codes:
		...
		if not os.path.exists(f"{path2save_references}{primer_code}"):
			...
			while count < 10:
				if len_pos == 5 or 'I' in primer_seq: 
					if count < 10 or 'I' in primer_seq: # os que nao tiverem 10 especies com query coverage acima de 85% e maximo len desse que eu citei, ja sao descartados. Para descartar, removo o diretório, depois penso um jeito melhor de fazer isso.. provavelemnte os filtered primers vao estar num arquivo csv, dai posso remover desse arquivo csv tambem
						import shutil
						shutil.rmtree(f"{path2save_references}{primer_code}")
						print('nao deu', primer_code)
					break
				else:
					
					count, selected_organisms = download_references(df_references, primer_code, primer_seq, max_len[len_pos], count, selected_organisms, path2save_references)
				len_pos += 1
